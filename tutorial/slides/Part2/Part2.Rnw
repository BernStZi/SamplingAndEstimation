...
head(sub1[,1:7])
s <- which(bm$Commune %in% c("Brecht",
                             "Grimbergen","As","Dinant"))
sub2 <- bm[s,]
sub2[,1:7]
@
}
...
\footnotesize{
<<>>=
sub3 <- subset(bm,Commune %in% c("Brecht", 
                                 "Grimbergen","As","Dinant"))
...
<<echo=T,eval=T,cache=CA>>=
fra <-data.frame(pre = sample(c(30,40,89,221,621),10000,
                              replace = T),bank = sample(100:99999,10000,replace = T))
fra[1:4,]
fra <- fra[order(fra[,1]),]
fra[1:4,]
@
