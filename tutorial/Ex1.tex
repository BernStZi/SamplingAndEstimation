...
\item Download the ESS dataset for Sweden (Survey Data and Sampling Design Data File (SDDF)) of the 5th round
...
\item Load the ESS dataset and the SDDF
...
\setcounter{enumi}{4}
\item Determine the sampling strategy (Inspect the variables \alert{PSU},\alert{STRATFY} and \alert{PROB})
\item Add the variable \R{N} for the population size to your data frame. \R{N} can be calulated by
\begin{equation*}
\R{N}=\R{dweight}*\R{pweight}*10000*n\text{,}
\end{equation*}
where $n$ refers to the sample size
\item Create a \R{svydesign} object from the dataset for Sweden using the \R{survey} package
...
\begin{itemize}
\item \R{id} specifies the identifier of PSU and SSU; \R{id$=$ \textasciitilde 0} or \R{id$=$\textasciitilde 1} stipulates a single stage sampling
...
\R{calibrate} & Calibration of a data set (uses the GREG-Estimator by default)\\
... & ...\\
\end{tabular}
