...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{#GREG with bounds}
\hlstd{sam.calBD}  \hlkwb{<-}
  \hlkwd{calibrate}\hlstd{(}\hlkwc{design} \hlstd{= sam.dsgn,}
            \hlkwc{formula} \hlstd{=} \hlopt{~} \hlstd{SEENCNT} \hlopt{+} \hlstd{EOYCNT} \hlopt{+} \hlstd{hosp.type}\hlopt{:}\hlstd{BEDS,}
            \hlkwc{population} \hlstd{= pop.tots,}
            \hlkwc{bounds} \hlstd{=} \hlkwd{c}\hlstd{(}\hlnum{0.5}\hlstd{,}\hlnum{2}\hlstd{),}
            \hlkwc{calfun}\hlstd{=}\hlstr{'linear'} \hlstd{)}
\end{alltt}


{\ttfamily\noindent\color{warningcolor}{\#\# Warning: package 'MASS' was built under R version 3.2.2}}\begin{alltt}
\hlcom{#ratio without bounds and with them}
\hlkwd{rbind}\hlstd{(}\hlkwc{noBD}\hlstd{=}\hlkwd{summary}\hlstd{(}\hlkwd{weights}\hlstd{(sam.cal)}\hlopt{/}\hlkwd{weights}\hlstd{(sam.dsgn)),}
      \hlkwc{BD}\hlstd{=}\hlkwd{summary}\hlstd{(}\hlkwd{weights}\hlstd{(sam.calBD)}\hlopt{/}\hlkwd{weights}\hlstd{(sam.dsgn)))}
\end{alltt}
\begin{verbatim}
##        Min. 1st Qu. Median   Mean 3rd Qu.  Max.
