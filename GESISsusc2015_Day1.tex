...
\title[Sampling Designs]{4. GESIS Summer School in Survey Methodology
...
{\centering \includegraphics[width=.85\linewidth]{graphs/beamer-SamSizeSRSprobs-1} 

}

\caption[Sample Sizes to Achieve CV's of 0]{Sample Sizes to Achieve CV's of 0.05 and 0.01 for N=50000}\label{fig:SamSizeSRSprobs}
\end{figure}
...
{\centering \includegraphics[width=.85\linewidth]{graphs/beamer-SamSizeSRSprobs2-1} 

}

\caption[Sample Sizes to Achieve Absolute Errors of 0]{Sample Sizes to Achieve Absolute Errors of 0.05 and 0.01 for N=50000}\label{fig:SamSizeSRSprobs2}
\end{figure}
...
{\centering \includegraphics[width=\linewidth]{graphs/beamer-SamSizeSRSpower-1} 

}

\caption[Sample Sizes for Type II Errors 0]{Sample Sizes for Type II Errors 0.1, 0.2, and 0.3 for N=50000}\label{fig:SamSizeSRSpower}
\end{figure}
...
% Fri Sep 04 14:35:05 2015
\begin{table}[ht]
\centering
\caption{Stratfication Table} 
...
\begin{alltt}
\hlstd{fct}  \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{x}\hlstd{)\{}\hlkwd{sum}\hlstd{(}\hlnum{1}\hlopt{/}\hlstd{x)\}}
\hlcom{#constraints}
\hlstd{hin0} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{x}\hlstd{)\{}\hlkwd{c}\hlstd{(   x} \hlopt{-} \hlnum{1}\hlopt{/}\hlstd{(Nh}\hlopt{+}\hlnum{0.01}\hlstd{)}       \hlcom{#upper size}
                       \hlstd{,} \hlnum{1}\hlopt{/}\hlstd{(}\hlnum{0.01}\hlopt{*}\hlstd{Nh}\hlopt{+}\hlnum{0.01}\hlstd{)} \hlopt{-} \hlstd{x}  \hlcom{#lower size}
                       \hlstd{, c}\hlopt{-}\hlstd{A}\hlopt{%*%}\hlstd{x)\}}             \hlcom{#precision}

\hlstd{ans} \hlkwb{<-} \hlkwd{constrOptim.nl}\hlstd{(}
  \hlkwc{par} \hlstd{=} \hlnum{1}\hlopt{/}\hlstd{(}\hlnum{0.42}\hlopt{*}\hlstd{Nh),}      \hlcom{# starting value}
  \hlkwc{fn} \hlstd{= fct,}               \hlcom{# objective function}
  \hlkwc{hin} \hlstd{= hin0,}
  \hlkwc{control.outer} \hlstd{=} \hlkwd{list}\hlstd{(}\hlkwc{eps}    \hlstd{=} \hlnum{1.e-09}\hlstd{,}
                       \hlkwc{mu0}    \hlstd{=} \hlnum{1e-01}\hlstd{,}
                       \hlkwc{method} \hlstd{=} \hlstr{"BFGS"}\hlstd{,}
                       \hlkwc{trace}  \hlstd{=} \hlnum{FALSE}
