...
\section{Variance Estimation for Complex Statistics}
...
\begin{lrbox}{\mysavebox}
<<GREG.Vest, message=FALSE, cache=TRUE, echo=3:4, dependson='GREG.calib'>>=
ori.opt <- options()
...
pest.ratio <- svytotal(~EXPTOTAL, sam.cal)/svytotal(~EXPTOTAL, sam.dsgn)
vest.ratio   <- vcov(svytotal(~EXPTOTAL, sam.cal))/vcov(svytotal(~EXPTOTAL, sam.dsgn))
...
\usebox{\mysavebox}
We find that $\hat{\tau}_\text{GREG} /  \hat{\tau}_\pi  = \Sexpr{round(pest.ratio,4)}$, but 
$ \Vest{\hat{\tau}_\text{GREG}} /  \Vest{\hat{\tau}_\pi}  = \Sexpr{round(vest.ratio,4)}$.
}
