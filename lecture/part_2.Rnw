...
\subtitle{Part 2: Complex Sampling Designs}
...
\section{Complex Sampling Designs - Cluster Sampling}
...
$\mu $ & =& \Sexpr{mean(clu)} & \\
$\V{\overline{y}}_{\text{SRS}}$ &=& \Sexpr{ SRS.var  }	&  SRS \\
$\V{\overline{y}_{\text{SRCS}}}_{\text{SRS}}$  &=& \Sexpr{ SRcS.var.col }   & SRCS  a \\
 $\V{\overline{y}_{\text{SRCS}}}_{\text{SRS}}$ &=& \Sexpr{ SRcS.var.row }   &  SRCS b \\
\end{tabular} \newline
...
\onslide*<1>{
<<StratPlot1.2,echo=FALSE,fig.width=12,fig.height=8,out.width='.85\\linewidth'>>=
set.seed(671)
n_i <- 4
sam.str <- sample(length(Nh),n_I)
par(xpd=TRUE)
...
<<StratPlot2.2,echo=FALSE,fig.width=12,fig.height=8,out.width='.85\\linewidth'>>=
<<StratPlot1.2>>
for(i in sam.str) {
...
\section{Complex Sampling Designs - Unequal Probability Sampling}
...
%See Yates & Grundy, 1953, S. 257, Equations 8, 11, and 12.
%When is Yates Grundy estimator larger than the Horvitz-Thompso Estimator
%
...
deff.bycol <- SRcS.var.col/SRS.var 
deff.byrow <- SRcS.var.row/SRS.var

deffs <- matrix(c(delta.bycol, delta.byrow, deff.bycol, deff.byrow),ncol=2,byrow=T)
dimnames(deffs) <- 
 list(c("$\\delta$","$\\deff$"), c("SRCS a", "SRCS b"))

deffs.tab <- xtable(deffs[-1,,drop=F], digits = 5, caption = "Design Effects")
	...
	\begin{itemize}
		\item Under cluster sampling and two-stage sampling we can use:\\
					...
					\begin{description}
						...
						\item[$\hat{\rho}$] as an appropriate estimator of $\rho$.
					\end{description}
		\item When $w_k$'s vary we have
					$$
					  \deffhatc = 1 + \left(b^{\ast} - 1\right) \hat{\rho}
					$$
					where $b^{\ast}= \frac{ \sum_{i \in \mathfrc{s}_{\RN{1}}} \left( \sum_{k \in \mathfrc{s}_i} w_{k} \right)^2 }{ \sum_{k \in \mathfrc{s} } w_k^2}$ is a kind of weighted average cluster size.% \cite{GablerEtal1999}.
	\end{itemize}
...
\onslide<1->{
  There are many different ways to estimate $\rho$.% \cite{Ganninger2009}.
  The classical ANOVA estimator is:
	\begin{align*}
		\hat{\rho} = \dfrac{ \widehat{MSB} -  \widehat{MSW}}{ \widehat{MSB}+(K-1) \widehat{MSW} }\;{,}
	\end{align*}%
where $ \widehat{MSB} =  (n_\RN{1}-1)^{-1} \sum_{i \in \mathfrc{s}_{\RN{1}}} n_i (\overline{y}_i - \overline{y})^2 $, $\widehat{MSW}  = (n-n_\RN{1})^{-1} \sum_{i \in \mathfrc{s}_{\RN{1}}} (n_i - 1) s^2_i$, and 
$$
 K = (n_\RN{1}-1)^{-1} \left( n - \sum_{ i \in \mathfrc{s}_\RN{1} }  \dfrac{n_i^2}{n} \right) \;{.}
$$
}
\onslide<2>{
\begin{itemize}
\item The model-based approach is widely used, because it often presents the only option for data users to estimate a design effect.
...
\begin{frame}{Comparison of Design Based \\ and Model Based Design Effects}
<<modelDeffEst, echo=FALSE,results='asis',message=FALSE>>=
#model-based
...
deffm.tab <- xtable(deffm, digits = 5, caption = "Intra-Class Correlation Coefficients and (Model) Design Effects")
