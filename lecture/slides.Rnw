...
Probability based samples - A finite set of possible samples each having a certain probability of being selected, given by the sampling design. The sampling design should be measurable, that is:
\beign{itemize}
...
Non-probability based samples - literally speaking the sampling method should select always the same sample if repeated. Often used for selection process that are less controlled and often to complex to be modeled. Thus Assumptions are made over the data itself (model-based inference).
...
A sampling algorithm is a set of rules used to select a sample from a population.

Two distinctions can be made:

Alorithms for Simple Random Sampling
All samples have the same probability of being selected.


Alorithms for Unequal Probability Sampling
Especially if the sample size is not random


A desiarble property is that is seqential, the  next draw only depends on the 


 Random Routes in a design that is not measureable 
...
<<LLN, echo=FALSE, cache=TRUE,warning=FALSE, results='hide'>>=
set.seed(1829)
