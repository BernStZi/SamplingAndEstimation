...
<<wrongVest, warning=FALSE, message=FALSE, results='hide', echo=FALSE>>=
load("D:/Teaching/SamplingAndEstimation/lecture/out_dg_.RData")
data(api)
Rsim <- dim(out)[1]
X  <- apipop[!is.na(apipop$enroll),]

q      <- qnorm(0.975)
tvalue <- mean(X$api00)

pest.av <- apply(out,c(2,3),mean)["Pest",,drop = FALSE]
vest.av <- apply(out,c(2,3),mean)["Vest",,drop = FALSE] 
pbias   <- apply(out,c(2,3),mean)["Pest",,drop = FALSE] / tvalue -1 
cr      <- array(NA, c(Rsim, dim(pbias)) )
dimnames(cr) <- list(1:Rsim, rownames(pbias), colnames(pbias))
  

for(d in dimnames(out)[[3]]){
  cr[,"Pest",d] <- out[,"Pest",d] - q*out[,"Vest",d] < tvalue &
    out[,"Pest",d] + q*out[,"Vest",d] > tvalue
  }

cr.av <- apply(cr,c(2,3),mean)

#pest and vest

Ests <- rbind(pest.av,vest.av)
colnames(Ests) <- 
  c("Naive", "Design", "Cal.Design", "Cal.Naive")

# pest bias and CR

Qsts <- rbind(pbias,cr.av)
colnames(Qsts) <- 
  c("Naive", "Design", "Cal.Design", "Cal.Naive")
rownames(Qsts) <- c("Bias", "CR")


xtable(Qsts, digits = 4)
...
<<ExEst, warning=FALSE, message=FALSE, results='asis', echo=FALSE>>=
tab.est <-
  xtable(Ests[,c("Naive", "Design", "Cal.Naive", "Cal.Design")],
         digits  = 4,
         caption = "Expected Values of Point and Variance Estimators")
print.xtable(tab.est,caption.placement = "top")

@

<<Qsts, warning=FALSE, message=FALSE, results='asis', echo=FALSE>>=
tab.qsts <-
xtable(Qsts[,c("Naive", "Design", "Cal.Naive", "Cal.Design")],
       digits  = 4,
       caption = "Bias and Coverage Rates")
print.xtable(tab.qsts, caption.placement = "top")
@
